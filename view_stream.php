...
if(isset($_GET['delid']))
{
    $sqldel = "DELETE FROM stream WHERE stream_id='" . $_GET['delid'] . "'";
    $qsqldel = mysqli_query($conn,$sqldel);
    echo mysqli_error($conn);
    if(mysqli_affected_rows($conn) == 1)
    {
        echo "<script>alert('Stream Record deleted successfully..');</script>";
        echo "<script>window.location='view_stream.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<div style="margin: 30px;">
  <div class="container" style="border: 1px solid;border-color: rgb(233, 229, 229); background-color: white;padding: 0;">
    <h1 style="text-align: center;  font-family: Roboto, sans-serif;color:rgb(255, 252, 252);background-color: rgb(101, 189, 227);">Student Accounts</h1>
    <table class="table">
  ...
  while($row = $result->fetch_assoc()) {
	?>
        <tr>
          ...
          <td><a href="view_stream.php?delid=<?php echo $row['stream_id']?>" class="btn btn-danger" onclick="return confirmdel()" style="margin-left: 5px;width: 80px;">Delete </a></td>
        </tr>
        <?php
	}
}
